...
def open_clone_dialog(
    ...
    *,
    ...
    ssh_button = QPushButton("Configurar SSH", url_row)
    url_layout.addWidget(ssh_button)
    ...
    def run_clone() -> None:
        ...
        uses_github_ssh = lower_url.startswith("git@github.com:") or lower_url.startswith("ssh://git@github.com/")
        if uses_github_ssh:
            cache = window.settings_data.get("github_ssh_cache", {})
            if not isinstance(cache, dict) or not bool(cache.get("authenticated", False)):
                answer = QMessageBox.question(
                    dialog,
                    "Clone SSH",
                    (
                        "Conexao SSH com GitHub ainda nao confirmada.\n"
                        "Deseja abrir a configuracao de chave SSH agora?"
                    ),
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
